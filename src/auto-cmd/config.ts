...
export async function readConfig(): Promise<Config> {
  try {
    ...
    console.log(`Error reading config file: ${(error as Error).message}`);
    // 如果配置文件不存在或为空，返回默认配置
...
export async function updateConfig(config: Config, executeCount: number = 0): Promise<void> {
  try {
    ...
    if (ext === '.json') {
      ...
      // 确保写入的内容不为空
      if (jsonContent.trim() === '{}') {
        console.log('Warning: Attempting to write empty config, using default config instead');
        const defaultConfig: Config = {
          time: ['9:30', '12:30', '19:00', '23:00'],
          mode: 'once',
          commands: []
    ...
    console.log(`Error updating config file: ${(error as Error).message}`);
    throw error;
  }
}
...
export async function updateJsConfig(config: Config, executeCount: number): Promise<void> {
  ...
  if (!commandsMatch) {
    console.log('Warning: commands array literal not found, skipping file update');
    return;
  }

  if (commandsMatch.index === undefined) {
    console.log('Warning: Could not determine commands array position, skipping file update');
    return;
  }
  ...
  // 如果数组为空，直接返回
  if (content[i] === ']') {
    console.log('Warning: commands array is empty, skipping file update');
    return;
  }
  ...
  if (arrayEnd === -1) {
    console.log('Warning: Invalid commands array structure, skipping file update');
    return;
  }
