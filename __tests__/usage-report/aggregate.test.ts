...
function ok(key: 'claude' | 'codex' | 'minimax', content: string, level: 'info' | 'warn' = 'info', summaryLine = `${key}-summary`): ProviderResult {
  return { status: 'ok', key, report: { title: `${key} title`, content, level, summaryLine } };
}

function err(key: 'claude' | 'codex' | 'minimax', message: string): ProviderResult {
...
describe('buildAggregateCard', () => {
  it('全部 info 时 level=info，标题含「汇总」', () => {
    const card = buildAggregateCard(
      [ok('claude', 'c', 'info'), ok('codex', 'c', 'info'), ok('minimax', 'c', 'info')],
      { nowMs: NOW }
    );
  ...
  it('任一 ok 且 warn 时 level=warn，标题含「告警」', () => {
    const card = buildAggregateCard(
      [ok('claude', 'c', 'warn'), ok('codex', 'c', 'info'), ok('minimax', 'c', 'info')],
      { nowMs: NOW }
    );
  ...
  it('失败 provider 不影响 level：1 error + 2 info 仍是 info', () => {
    const card = buildAggregateCard(
      [err('claude', 'keychain 不可用'), ok('codex', 'c', 'info'), ok('minimax', 'c', 'info')],
      { nowMs: NOW }
    );
    expect(card.level).toBe('info');
  });

  it('失败 provider 不影响 level：1 error + 1 warn + 1 info 仍是 warn', () => {
    const card = buildAggregateCard(
      [err('claude', 'x'), ok('codex', 'c', 'warn'), ok('minimax', 'c', 'info')],
      { nowMs: NOW }
    );
    expect(card.level).toBe('warn');
  });

  it('三块都在，每块标题独占一行，块间用空行分隔', () => {
    const card = buildAggregateCard(
      [ok('claude', 'claude-body'), ok('codex', 'codex-body'), ok('minimax', 'minimax-body')],
      { nowMs: NOW }
    );
  ...
  it('ok 的块正文原样拼接（不二次格式化）', () => {
    const card = buildAggregateCard(
      [ok('claude', '**账号**：pro\n  5 小时：30%'), ok('codex', 'x'), ok('minimax', 'y')],
      { nowMs: NOW }
    );
  ...
  it('失败的 provider 块显示「⚠️ 获取失败：<message>」', () => {
    const card = buildAggregateCard(
      [err('claude', '无法获取凭证'), ok('codex', 'c'), ok('minimax', 'c')],
      { nowMs: NOW }
    );
    expect(card.content).toContain('🟦 **Claude**');
  ...
  it('顶部带当前时间 header', () => {
    const card = buildAggregateCard([ok('claude', 'c'), ok('codex', 'c'), ok('minimax', 'c')], { nowMs: NOW });
  ...
  it('summaryLine：ok 形如 key=<report.summaryLine>，error 形如 key=ERROR:<message>', () => {
    const card = buildAggregateCard(
      [ok('claude', 'c', 'info', 'alert=false'), err('codex', '401'), ok('minimax', 'c', 'info', 'mm alert=true')],
      { nowMs: NOW }
    );
